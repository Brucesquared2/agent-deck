...
| A5  | Authority must be singular |
| A6  | Lineage must be preserved |
| A7  | Agents are workers, not truth |
| A8  | Reflections are recovery surfaces, not authority |
| A9  | Narrative and payload are paired views of one object |
...
| `50-maps/MEMORY_MAP.md` | Numbered memory taxonomy |
| `60-signals/signal-lang.md` | Versioned Signal-Lang event specification |
| `60-signals/signal-schema.json` | Formal signal/event payload validation |
| shared agent contracts | Behavioral boundaries |
...
## 12. Provisional canon artifact set

The following are the provisional authority artifacts (this canon set):

| File | Role |
|------|------|
| `WAREHOUSE_SPECIFICATION.md` | Root authority candidate |
...
| `WAREHOUSE_CANONICALIZATION_REVIEW.md` | Bruce decision surface |
| `CANONIZATION_DECISIONS.md` | Explicit decision records |
| `TAXONOMY_MIGRATION_PLAN.md` | Taxonomy conflict resolution |
| `SIGNAL_LANG_MIGRATION_PLAN.md` | Signal format conflict resolution |
| `DEPRECATION_POLICY.md` | Deprecation governance |
...
- Bruce approves the decision packet (see `WAREHOUSE_CANONICALIZATION_REVIEW.md`)
